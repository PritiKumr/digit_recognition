...
def convolution_layer(input, filters, winSize, initializer, name):
    ...
    return tf.layers.conv2d(
        ...
        kernel_initializer=initializer['he_rec'],
        name=name)
...
def cnn_model_fn(features, labels, mode, params):
    ...
    with tf.name_scope('Input'):
        ...
        tf.summary.image('Input-Images', input, 10)
...
def main():
    ...
    learn_rate = 0.001
    cnn_layers = 2
    model_dir = '../models/cnn_model_5epochs'
    ...
    with np.nditer(train_labels, op_flags=['readwrite']) as it:
        for x in it:
            x[...] = x % 2
    
    eval_data = mnist.test.images  # Returns np.array
    eval_labels = np.asarray(mnist.test.labels, dtype=np.int32)
    with np.nditer(eval_labels, op_flags=['readwrite']) as it:
        for x in it:
            x[...] = x % 2
    ...
    while True:
        option= input("1. Make Predictions \n2. Quit\n")
        if(option=="1"):
            image_path = input("Enter path of the file (eg. ../data/1.jpg): ")
            image = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
            image = cv2.resize(image, (28, 28))
            ret, bin_image = cv2.threshold(image, 127, 255, cv2.THRESH_BINARY_INV)
            flatten = np.asarray([bin_image.flatten() / 255.0], dtype=np.float32)
            ...
            result = "even" if predicted_classes[0] == 0 else "odd"
